function [groups] = group_matching3(T2D,N,Nstep,NSh,z,Nmax,thresh,H,W)
...
for i=1:Nstep:H-N+1
    for j=1:Nstep:W-N+1
        ...
        for k=uPos:Nstep:dPos-N+1
            for l=lPos:Nstep:rPos-N+1
                if(l==j && k==i)
                    continue;
                end
                cBlock=tBlocks(((k-1)/3)*((W-N)/3+1)+(l-1)/3+1).tBlock;
                cPos=tBlocks(((k-1)/3)*((W-N)/3+1)+(l-1)/3+1).pos;
